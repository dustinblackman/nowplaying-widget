...
describe("Backend", () => {
  ...
  it("GET /user/test (before auth)", () => {
    return request.get("/user/test").then(res => {
  ...
  it("GET /authorize/test", () => {
    return request.get("/authorize/test").then(res => {
      expect(res.status).to.equal(302);
  ...
  it("GET /authorize/callback?state=test", () => {
    return request.get("/authorize/callback?state=test").then(res => {
      expect(res.status).to.equal(302);
  ...
  it("GET /api/playing/test", () => {
    return request.get("/api/playing/test").then(res => {
      expect(res.status).to.equal(200);
